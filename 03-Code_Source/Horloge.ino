...
volatile boolean choix_minute= true;

volatile int compteur = 0; 
int ancien_compteur = 0;
unsigned long chrono,tempo = 0;
volatile boolean point_heure = false;   // pour afficher le point entre les heures
...
void loop() {
  ...
  if (bouton == 0) { // si on appuie sur le bouton
    ...
    if (ancien_bouton == 0)
    {
      ...
      if (choix_minute== true){compteur=heures;}else {compteur=minutes;}
    ...
    if (choix_minute==false){compteur=heures+compteur;}else {compteur=minutes+compteur;}
    Serial.println(compteur);             // on l'affiche
    if (choix_minute == true) { // gestion des depassement de minutes
      if ((compteur > 60) ||  (compteur <0)){rtc.setMinutes(0);compteur=0;}
      else {rtc.setMinutes(compteur);compteur=0;}
    }
    else { // gestion des dépassements des heures
      if ((compteur > 24)||(compteur <0)) {rtc.setHours(0);compteur=0;}
      else {rtc.setHours(compteur);compteur=0;}      
    }
  }
  ...
  if (heures <10){sprintf (Message,"0%d",heures);}  // pour afficher le 0 devant les heures <10
  else{sprintf (Message,"%d",heures); }
  
  if (point_heure== true) {sprintf (Message,"%s:",Message);}  // pour faire clignoter le '.' entre les heures et les minutes
  else {sprintf (Message,"%s ",Message);}
  
  if (minutes <10){sprintf (Message,"%s0%d",Message,minutes);} // pour afficher un 0 devant les minutes si elles sont <10
  else {sprintf (Message,"%s%d",Message,minutes);}
