...
# modified for pyboard and another blue board and red board by Benoit DUFLOS
...
class TFT(object) :
  ...
  COLORS=[RED,MAROON,GREEN,FOREST,BLUE,NAVY,CYAN,YELLOW,PURPLE,WHITE,GRAY,]
  ...
  def text( self, aPos, aString, aColor, aFont, aSize = 1, nowrap = False ) :
    ...
    for c in aString:
      ...
      px += width
      #We check > rather than >= to let the right (blank) edge of the character print off the right of the screen.
  ...
  def initb2( self ) :
    ...
    self._size = (ScreenSize[0] + 2, ScreenSize[1] + 2)
    self._offset[0] = 2
    self._offset[1] = 2
    self._reset()
